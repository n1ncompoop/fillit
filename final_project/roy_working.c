...
/*   Updated: 2018/04/05 23:43:48 by pnarayan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include <unistd.h>
#include <stdio.h>
...
char    **gen_map(int x)//MEMORY GENERATION
{
    ...
    while (i < x)
    {
        if (!(map[i] = (char *)malloc(sizeof(char) * x)))
            return (NULL);
        ft_memset(map[i], '.', x);
        map[i][x] = '\0';
        i++;
...
int     *app_end(int *pce_lst, int *new_pce)//APPENDS NEXT PIECE TO CURRENT CHAIN OF PIECES
{
    ...
    cur_cnt = 0;
    i = 0;
    j = 2;// starts from  2 because ROY needs 3 coords instead of 4 (which includes 00)
    if ((pce_lst[cur_cnt]) != -42)
    {
        while (pce_lst[cur_cnt] != -42)
            cur_cnt++;
    }
    if (!(new_lst = (int *)malloc(sizeof(int) * (cur_cnt + 8))))// 8 or 7 ?????
        return(NULL);
    while (pce_lst[i] != -42)
    {
        new_lst[i] = pce_lst[i];
